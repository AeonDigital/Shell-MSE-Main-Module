...
mseTMPInstallationPath="${mseHOME}/apps/myShellEnv"
    ...
    if [ -d "${mseTMPInstallationPath}" ]; then
      ...
      if [ ! -d "${mseTMPInstallationPath}" ]; then
        ...
        if [ $? != 0 ] || [ ! -d "${mseTMPInstallationPath}/src" ]; then
          ...
          git -C "${mseTMPInstallationPath}" config --local user.email "mse@myenv.shell"
...
if [ "${mseIsOK}" == "0" ]; then
  ...
  if [ -f "${mseAtualShellRCPath}" ]; then
    ...
    mseMyEnvShellInitiCode+="mseHOME=\$([ ! -z \"\${XDG_MAIN_HOME}\" ] && echo \"\${XDG_MAIN_HOME}\" || echo \"\${HOME}\")\n"
    mseMyEnvShellInitiCode+="if [ -f \"\${mseHOME}/apps/myShellEnv/src/mse.sh\" ]; then\n"
    mseMyEnvShellInitiCode+="  . \"\${mseHOME}/apps/myShellEnv/src/mse.sh\" || true\n"
  ...
  mseTMPInstallMessage+=("mseHOME=\$([ ! -z \"\${XDG_MAIN_HOME}\" ] && ${mseTtlColor}echo${mseNONE} \"\${XDG_MAIN_HOME}\" || ${mseTtlColor}echo${mseNONE} \"\${HOME}\")")
  mseTMPInstallMessage+=("${mseTtlColor}if${mseNONE} [ -f \"\${mseHOME}/apps/myShellEnv/src/mse.sh\" ]; ${mseTtlColor}then${mseNONE}")
  mseTMPInstallMessage+=("  . \"\${mseHOME}/apps/myShellEnv/src/mse.sh\" || ${mseTtlColor}true${mseNONE}")
