...
test_mse_check_isCommandExists() {
  ...
  testResult=$(mse_exec_validate mse_check_isCommandExists)
  testExpected="0"
  ...
  mse_exec_validate mse_check_isCommandExists &> /dev/null
  ...
  mseTest=$(mse_exec_validate mse_check_isCommandExists && printf " :: ${MSE_GLOBAL_RETURN}" || printf " :: ${MSE_GLOBAL_LASTERR}")
  testResult="${mseTest}"
  testExpected="0 :: Parameter \"Command\" is required"
