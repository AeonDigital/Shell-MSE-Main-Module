...
# Se omitido usará o padrão "${HOME}/apps/myShellEnv" ou
# "${XDG_MAIN_HOME}/apps/myShellEnv".
#
# @param bool $2
# Opcional.
...
myShellEnvStart() {
  ...
  if [ "$(type -t "mse_mmod_ini_registerModule")" == "function" ]; then
    mseReturn=0
  else
    ...
    local msePathToRegisterModuleFunction="${mseInstallationPath}/src/scripts/09_mmod/mse_mmod_ini_registerModule.sh"
    local msePathToLoadLocaleLabelsFunction="${mseInstallationPath}/src/scripts/09_mmod/mse_mmod_ini_loadLocaleLabels.sh"
    ...
    if [ ! -f "${msePathToRegisterModuleFunction}" ] || [ ! -f "${msePathToLoadLocaleLabelsFunction}" ]; then
      ...
      if [ "${MSE_GLOBAL_ACTIVE}" == "1" ] || [ "$2" == "1" ]; then
        ...
        mse_mmod_ini_registerModule "Shell-MSE-Main-Module" "${mseInstallationPath}/src"
        ...
        for mseModuleName in "${!MSE_AVAILABLE_MODULES[@]}"; do
          if [ "${MSE_AVAILABLE_MODULES[${mseModuleName}]}" == "1" ]; then
            mse_mmod_ini_registerModule "${mseModuleName}" "${mseInstallationPath}/${mseModuleName}/src"
          fi
        done
...
myShellEnvRunTests() {
  ...
  if [ $? == 0 ] && [ "$(type -t "mse_mmod_ini_registerModule")" == "function" ]; then
    #
    ...
    if [ ! -d "${mseTestModulePath}" ]; then
      ...
      mse_mmod_ini_registerModule "Shell-MSE-UnitTest" "${mseInstallationPath}/Shell-MSE-UnitTest/src"
