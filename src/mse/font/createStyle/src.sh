  ...
  if [ $# -lt 4 ]; then
      ...
      for mseUParam in "${MSE_LAST_FUNCTION_RETURN[@]}"; do
        if [ "${mseUParam}" != "" ]; then
          ...
          if [ "${mseIsResult}" == "1" ]; then
            mseIsCheck=$(mse_array_ifhas_value "${mseUParam}" "MSE_MD_ICOLOR_AVAILABLE_FONT_ATTRIBUTE_CODES")
          ...
          if [ "${mseTmpAttr}" != "" ]; then
            mseIsCheck=$(mse_array_ifhas_value "${mseTmpAttr}" "mseValidAttr")
    ...
    if [ "${mseTmpBackground}" != "" ]; then
      ...
      case "${mseColorSystem}" in
        4)
          if [ "${mseIsResult}" == "1" ]; then
            mseIsCheck=$(mse_array_ifhas_value "${mseTmpBackground}" "MSE_MD_ICOLOR_AVAILABLE_BGCOLOR_CODES")
            if [ "${mseIsCheck}" == "1" ]; then
    ...
    if [ "${mseTmpForeground}" != "" ]; then
      ...
      case "${mseColorSystem}" in
        4)
          if [ "${mseIsResult}" == "1" ]; then
            mseIsCheck=$(mse_array_ifhas_value "${mseTmpForeground}" "MSE_MD_ICOLOR_AVAILABLE_COLOR_CODES")
            if [ "${mseIsCheck}" == "1" ]; then
