...
mse_man_process_section_data() {
  ...
  if [ "${mseTargetSectionName}" != "" ] && [ $(mse_is_int "${mseTargetSectionLevel}") == "1" ]; then
    ...
    if [ "${mseTargetSectionContent}" != "" ]; then
      ...
      MSE_MAN_SECTION_DATA["title"]=$(mse_normalize_string "${msePartTitle}")
      MSE_MAN_SECTION_DATA["summary"]=$(mse_normalize_string "${msePartSummary}")
      MSE_MAN_SECTION_DATA["description"]=$(mse_normalize_string "${msePartDescription}")
      MSE_MAN_SECTION_DATA["subsections"]=$(mse_normalize_string "${msePartSubsections}")
    fi
  fi
}